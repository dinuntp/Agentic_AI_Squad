...
export default function AgentStudio() {
  ...
  // Per-agent state
  const [stepStatuses, setStepStatuses] = useState({});   // { agentId: 'pending'|'running'|'completed'|'failed' }
  const [outputs, setOutputs] = useState({});              // { agentId: string }
  const [contexts, setContexts] = useState({});            // { agentId: object } — stored after complete
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
        ...
        {sortedAgents.map((agent, i) => (
          <AgentCard
            ...
            agent={agent}
            stepIndex={i}
            isActive={i === currentIdx && !allDone}
            isLocked={i > currentIdx && !allDone}
            agentStatus={getStatus(agent.id)}
