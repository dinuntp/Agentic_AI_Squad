...
async function gatherJiraContext(project, jiraStory) {
  // Helper: format a manually-entered story stored in project.stories
  function formatManualStory(s, note = '') {
    return `## Story: ${s.key}
**Summary:** ${s.summary}
**Source:** Manual entry${note ? ` (${note})` : ''}

### Description
${s.description || 'No description provided.'}`;
  }

  // Always check project.stories first — covers manual entries and Jira fallback
  const manualStory = (project.stories || []).find(s => s.key === jiraStory);

  if (!jira.isConfigured(project.jira)) {
    return manualStory
      ? formatManualStory(manualStory)
      : `Jira Story: ${jiraStory}\n(Jira not configured — no story details available)`;
  }
...
${story.acceptanceCriteria || 'No acceptance criteria defined.'}`;
    ...
    // Jira unreachable or story not found — fall back to manual story if available
    return manualStory
      ? formatManualStory(manualStory, `Jira unavailable: ${err.message}`)
...
async function parseAndSaveFiles(output, project, runId, emit) {
  ...
  // Ensure output directory exists — do NOT delete existing files so
  // enhancement runs preserve files the agent didn't touch.
...
async function pushToGitHub(project, storyKey, savedFiles, baseDir, leadOutput, devOutput, emit) {
  if (!github.isConfigured(project.git)) {
    emit('pipeline_log', { message: 'GitHub not configured — skipping PR creation.' });
    return null;
  }

  const slug = storyKey.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
  const branchName = `feature/${slug}`;
  const baseBranch = project.git.branch || 'main';

  // Create branch
  try {
    emit('pipeline_log', { message: `🔀 Creating GitHub branch: ${branchName}` });
    await github.createBranch(project.git, branchName, baseBranch);
    emit('pipeline_log', { message: `✓ Branch created: ${branchName}` });
  } catch (err) {
    if (err.message.includes('already exists') || err.message.includes('422')) {
      emit('pipeline_log', { message: `Branch ${branchName} already exists — updating files on it.` });
    } else {
  ...
  // Push each generated file to the branch
  let pushed = 0;
  for (const filePath of savedFiles) {
    try {
      const fullPath = join(baseDir, ...filePath.split('/'));
      const content = await readFile(fullPath, 'utf-8');
      await github.createOrUpdateFile(
        project.git,
        filePath,
        content,
        `feat(${storyKey}): add ${filePath}`,
        branchName
      );
      pushed++;
      emit('pipeline_log', { message: `✓ Pushed: ${filePath}` });
    } catch (err) {
      emit('pipeline_log', { message: `⚠ Failed to push ${filePath}: ${err.message}` });
    }
  }

  if (pushed === 0) {
    emit('pipeline_log', { message: '⚠ No files pushed — skipping PR creation.' });
    return null;
  }

  // Build PR body from agent outputs
  const leadSummary = leadOutput
    ? leadOutput.slice(0, 1200) + (leadOutput.length > 1200 ? '\n\n_(truncated)_' : '')
    : '_Lead Agent output not available._';

  const devSummary = devOutput
    ? devOutput.slice(0, 800) + (devOutput.length > 800 ? '\n\n_(truncated)_' : '')
    : '_Developer Agent output not available._';

  const prBody = `## 🤖 AI-Generated Implementation — ${storyKey}

> This pull request was automatically created by the **Agentic AI Squad** pipeline.
...
### Developer Agent Summary
${devSummary}

---
_Generated by Agentic AI Squad · Story: ${storyKey}_`;

  // Create the PR
  try {
    const pr = await github.createPullRequest(project.git, {
      title: `feat(${storyKey}): AI-generated implementation`,
      body: prBody,
      head: branchName,
      base: baseBranch
    });
    emit('pipeline_log', { message: `🎉 Pull Request created: ${pr.url}` });
    emit('pr_created', { url: pr.url, number: pr.number, title: pr.title, branch: branchName });
    return pr;
  } catch (err) {
    emit('pipeline_log', { message: `⚠ PR creation failed: ${err.message}` });
    return null;
  }
}
...
${gitContext}`;
  ...
  for (let i = 0; i < sortedAgents.length; i++) {
    ...
    try {
      ...
      // After developer agent: save files, execute them, push to GitHub + create PR
      if (agent.type === 'developer-agent') {
        ...
        if (savedFiles.length > 0) {
          ...
          const leadOutput = pipelineContext.previousOutputs['lead-agent']?.output || '';
          const pr = await pushToGitHub(project, run.jiraStory || jiraStory, savedFiles, baseDir, leadOutput, output, emit);
          if (pr) {
            run.prUrl = pr.url;
            run.prNumber = pr.number;
            run.prBranch = `feature/${(run.jiraStory || jiraStory).toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-')}`;
          }
        }
      }
...
export async function runSingleAgent(agent, project, storyKey, previousContext, emit) {
  ...
  if (agent.type === 'developer-agent') {
    ...
    if (savedFiles.length > 0) {
      ...
      // Push to GitHub and open a PR
      const leadOutput = pipelineContext.previousOutputs['lead-agent']?.output || '';
      await pushToGitHub(project, storyKey, savedFiles, baseDir, leadOutput, output, emit);
    }
  }
